# -*- coding: utf-8 -*-
"""
Created on Tue Nov  6 12:08:19 2018
...
def getMaxDocID(urlListCsvDf,urlListDict):
    if len(urlListCsvDf) > 0:
        docIDCsv = urlListCsvDf['DOCID'].max()+1 #max(int(d['DOCID']) for d in urlDict.values()) + 1
    else:
        docIDCsv = 1
...
def findUrls(parentUrl,UrlArr,depth,urllistcsv):
    ...
    rp.set_url(getSourceURL(parentUrl)+"robots.txt")
    rp.read()
                            ...
                            if (link[0] != '#' and link[:4] != 'mail' and link[-4:] != '.pdf' and 'javascript' not in link):
                                ...
                                if link[:4] == 'http':
                                    ...
                                    for m in re.finditer("\.\./",link):
                                        x = m.end(0)
                                    newLink = getSourceURL(url)+link[x:]   
                                elif link[:1] == '/':
                                    newLink = getSourceURL(url)+link[1:]
                                else:
                                    newLink = getSourceURL(url)+link
                                
                                if newLink not in urlDict.keys():
                                    if getSourceURL(newLink) == getSourceURL(url):
...
def createContentFiles(soupData,url,urlDict):
    ...
    for word in metaWords:
        file.write(ps.getStem(word)+"\n")
    for j in range(len(titles)):
        line = tk.tokenize(titles[j].get_text())
        for word in line:
            file.write(ps.getStem(word)+"\n")
    for k in range(len(h1)):
        line = tk.tokenize(h1[k].get_text())
        for word in line:
            file.write(ps.getStem(word)+"\n")
    for l in range(len(h2)):
        line = tk.tokenize(h2[l].get_text())
        for word in line:
            file.write(ps.getStem(word)+"\n")
    for i in range(len(paragraphs)):
        line = tk.tokenize(paragraphs[i].get_text())
        for word in line:
            file.write(ps.getStem(word)+"\n")
...
def createMetadataFiles(soupData,url,urlDict):
    ...
    for tag in soupData.find_all("meta"):
        if tag.get("property", None) == "og:title":
            line.append(tk.tokenize(tag.get("content", None)))        
        if tag.get("name",None) == "keywords":
            line.append(tk.tokenize(tag.get("content", None)))
        if tag.get("name",None) == "description":
            line.append(tk.tokenize(tag.get("content", None)))    
    for word in list(itertools.chain.from_iterable(line)):
        wordArr.append(ps.getStem(word))
